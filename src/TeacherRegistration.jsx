...
// Correcting the import paths to be relative to the current file
    ...
    const onGoogleSignIn = async (e) => {
        ...
        if (!isSigningIn) {
            ...
            try {
                ...
                // We now pass 'teacher' as the role when signing in.
                await doSignInWithGoogle('teacher');
        ...
        <div className="min-h-screen w-full flex flex-col bg-white text-gray-900">
            <header className="border-b border-gray-200 w-full">
                <nav className="flex items-center justify-between p-4 md:px-6 max-w-screen-xl mx-auto w-full">
                    ...
                    <Link to="/StudentRegistration" className="rounded-md px-5 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-100">
                        Student Sign-In
                    </Link>
                </nav>
            </header>
            ...
            <main className="flex-grow flex items-center justify-center w-full bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
                <div className="w-full max-w-md space-y-8 bg-white p-8 sm:p-10 rounded-xl shadow-lg">
                    <div className="text-center">
                        <h2 className="text-3xl font-bold text-gray-900 tracking-tight">Teacher Sign-In</h2>
                        <p className="mt-2 text-sm text-gray-600">Welcome! Access your teacher dashboard.</p>
                    </div>
